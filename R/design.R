...
design <- function(
	...
	if (design_info$is_factorial) {
		outdesign$book <- apply_factor_names(
			outdesign$book,
			fac.names,
			"factorial"
		)
...
#' @param fac.sep The separator used by `fac.names`. Used to combine factorial design levels. If a vector of 2 levels is supplied, the first separates factor levels and label, and the second separates the different factors.
#' @param buffer The type of buffer. One of edge, row, column, double row,
#' double column, blocks (internal boundaries between blocks), or double block/entire block/full block.
...
des_info <- function(
	...
	if (design_info$is_factorial && !is.null(fac.names)) {
		...
		if (is.list(fac.names)) {
			...
			for (i in seq_along(fac.names)) {
				if (factor_cols[i] %in% names(des)) {
					...
					# Rename column
					names(des)[names(des) == factor_cols[i]] <- names(
						fac.names
					)[i]
				}
			}
		...
		(design_info$type == "split" || design_info$type == "strip") &&
			!is.null(fac.names)
	) {
...
#'
#' @noRd
create_agricolae_design <- function(
	parsed_type,
	treatments,
	reps,
	sub_treatments,
...
build_design <- function(
	...
	des <- switch(
		...
		factorial_crd = build_factorial_crd(
			design.obj$book,
			nrows,
			ncols,
			fac.sep
		),
		factorial_rcbd = build_factorial_rcbd(
			design.obj$book,
			nrows,
			ncols,
			brows,
...
build_factorial_crd <- function(design_book, nrows, ncols, fac.sep) {
	...
	design_book$treatments <- construct_factorial_labels(
		design_book,
		3,
		fac.sep
	)
...
build_split <- function(design_book, nrows, ncols, brows, bcols, byrow) {
	...
	if (!byrow) {
		des[, c("row", "col", "block")] <- des[
			order(des$block, des$col, des$row),
			c("row", "col", "block")
		]
	}
...
normalise_agricolae_book <- function(design_book, design_info) {
	...
	if (
		identical(design_info$type, "split") ||
			identical(design_info$base, "split") ||
			identical(design_info$type, "strip") ||
			identical(design_info$base, "strip")
	) {
