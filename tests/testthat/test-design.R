...
test_that("RCBD designs are supported", {
	...
	expect_equal(d2$satab[3], "Block stratum                           3\n")
	expect_snapshot_output(d2$satab)
	vdiffr::expect_doppelganger(
		title = "RCBD plot produced",
		autoplot(d2),
		variant = ggplot2_variant()
	)
})
...
test_that("LSD designs are supported", {
	...
	expect_equal(d3$satab[6], "Residual                                6\n")
	expect_snapshot_output(d3$satab)
	vdiffr::expect_doppelganger(
		title = "LSD plot produced",
		autoplot(d3),
		variant = ggplot2_variant()
	)
})
...
test_that("Strip plot designs are supported", {
	...
	by_block_row <- aggregate(
		wp_treatments ~ block + row,
		data = d_strip$design,
		FUN = function(x) length(unique(x))
	)
	expect_true(all(by_block_row$wp_treatments == 1))

	by_block_col <- aggregate(
		sub_treatments ~ block + col,
		data = d_strip$design,
		FUN = function(x) length(unique(x))
	)
...
test_that("Split plot designs with double row blocks are supported", {
	...
	vdiffr::expect_doppelganger(
		title = "Split plot double row blocks",
		autoplot(d4.2),
		variant = ggplot2_variant()
	)
})
...
test_that("Crossed CRD designs are supported", {
	...
	expect_design_df_has_cols(
		d5$design,
		c("plots", "reps", "A", "B", "treatments")
	)
	expect_design_df_ends_with(d5$design, "treatments")
	expect_equal(d5$satab[5], "A:B                                     2\n")
	expect_snapshot_output(d5$satab)
	vdiffr::expect_doppelganger(
		title = "Factorial CRD plot no space sep",
		autoplot(d5),
		variant = ggplot2_variant()
	)
...
test_that("Crossed RCBD designs are supported", {
	...
	expect_design_df_has_cols(
		d6$design,
		c("plots", "block", "A", "B", "treatments")
	)
	expect_equal(d6$satab[8], "Residual                                10\n")
	expect_snapshot_output(d6$satab)
	vdiffr::expect_doppelganger(
		title = "Factorial RCBD plot produced",
		autoplot(d6),
		variant = ggplot2_variant()
	)
})
...
test_that("Crossed RCBD designs with row blocks are supported", {
	...
	vdiffr::expect_doppelganger(
		title = "Factorial RCBD plot with row blocks",
		autoplot(d6.1),
		variant = ggplot2_variant()
	)
})
...
test_that("Crossed RCBD designs with double row blocks are supported", {
	...
	expect_equal(d6.2$satab[8], "Residual                                10\n")
	vdiffr::expect_doppelganger(
		title = "Factorial RCBD plot double row blocks",
		autoplot(d6.2),
		variant = ggplot2_variant()
	)
})
...
test_that("Crossed RCBD designs with square blocks are supported", {
	...
	vdiffr::expect_doppelganger(
		title = "Factorial RCBD plot square blocks",
		autoplot(d6.3),
		variant = ggplot2_variant()
	)
})
...
test_that("Crossed LSD designs are supported", {
	...
	expect_design_df_has_cols(
		d7$design,
		c("plots", "row", "col", "A", "B", "treatments")
	)
	expect_design_df_ends_with(d7$design, "treatments")
	expect_equal(d7$satab[3], "Row                                     5\n")
	expect_snapshot_output(d7$satab)
	vdiffr::expect_doppelganger(
		title = "Factorial LSD plot with sep",
		autoplot(d7),
		variant = ggplot2_variant()
	)
})
...
test_that("Crossed LSD designs with names are supported", {
	d7.1 <- design(
		...
		nrows = 6,
		ncols = 6,
		fac.names = list(N = c(50, 100, 150), W = c("I", "R")),
		seed = 42,
		quiet = TRUE
	)
	...
	expect_design_df_has_cols(
		d7.1$design,
		c("plots", "row", "col", "N", "W", "treatments")
	)
	expect_equal(d7.1$satab[3], "Row                                     5\n")
	vdiffr::expect_doppelganger(
		title = "Factorial LSD with names",
		autoplot(d7.1),
		variant = ggplot2_variant()
	)
})
...
test_that("Crossed LSD designs with names and separator are supported", {
	...
	expect_equal(d7.2$satab[3], "Row                                     5\n")
	vdiffr::expect_doppelganger(
		title = "Factorial LSD plot names and sep",
		autoplot(d7.2),
		variant = ggplot2_variant()
	)
})
...
test_that("Nested designs are supported", {
	...
	d8 <- design(
		type = "lsd",
		treatments = c("A1", "A2", "A3", "A4", "B1", "B2", "B3"),
		nrows = 7,
		ncols = 7,
		seed = 42,
		quiet = TRUE
	...
	expect_equal(d8$satab[6], "Residual                                30\n")
...
test_that("3 way factorial designs are possible", {
	...
	expect_design_df_has_cols(
		d9$design,
		c("plots", "reps", "A", "B", "C", "treatments")
	)
	expect_equal(d9$satab[6], "A:B                                     1\n")
	expect_equal(d9$satab[9], "A:B:C                                   1\n")
	expect_snapshot_output(d9$satab)
	vdiffr::expect_doppelganger(
		title = "3 way factorial",
		autoplot(d9),
		variant = ggplot2_variant()
	)
	...
	expect_design_df_has_cols(
		d9.1$design,
		c("plots", "reps", "X", "Y", "Z", "treatments")
	)
	expect_equal(d9.1$satab[6], "X:Y                                     1\n")
	expect_equal(d9.1$satab[9], "X:Y:Z                                   1\n")
	expect_snapshot_output(d9.1$satab)
	vdiffr::expect_doppelganger(
		title = "3 way factorial with names",
		autoplot(d9.1),
		variant = ggplot2_variant()
	)
})
...
test_that("Adding names to 3 way factorial designs works", {
	...
	expect_design_df_has_cols(
		d9.2$design,
		c("plots", "block", "X", "Y", "Z", "treatments")
	)
	...
	expect_equal(d9.2$satab[3], "Block stratum                           2\n")
	expect_equal(d9.2$satab[12], "Residual                                14\n")
	expect_snapshot_output(d9.2$satab)
	vdiffr::expect_doppelganger(
		title = "3 way rcbd factorial with names",
		autoplot(d9.2),
		variant = ggplot2_variant()
	)
})
...
test_that("split plot allows a character vector for factor names", {
	...
	vdiffr::expect_doppelganger(
		title = "Split plot with vector names",
		autoplot(d11),
		variant = ggplot2_variant()
	)
})
...
test_that("split plot produces warning when incorrect number of treatment labels given", {
	expect_warning(
		design(
			...
			bcols = 2,
			seed = 42,
			fac.names = list(Water = "ABC", N = 1:4)
		),
	...
	expect_warning(
		design(
			...
			bcols = 2,
			seed = 42,
			fac.names = list(Water = c("A", "B"), N = 1:10)
		),
	...
	expect_warning(
		design(
			...
			bcols = 2,
			seed = 42,
			fac.names = list(Water = c("A", "B"), N = 1:4, Another = 1:5)
		),
...
test_that("factorial designs produce warnings when incorrect number of treatment labels given", {
	expect_warning(
		design(
			...
			brows = 6,
			bcols = 1,
			fac.names = list(Water = c("A", "B"), N = 1:2),
			quiet = TRUE
		),
		"Water must contain the correct number of elements. Elements have not been applied."
	)
	...
	expect_warning(
		design(
			...
			brows = 6,
			bcols = 1,
			fac.names = list(Water = c("A", "B", "C"), N = 1),
			quiet = TRUE
		),
		"N must contain the correct number of elements. Elements have not been applied."
	)
	...
	expect_warning(
		design(
			...
			brows = 6,
			bcols = 1,
			fac.names = list(Water = c("A", "B", "C"), N = 1:2, Another = 1:10),
			quiet = TRUE
		),
		"fac.names contains 3 elements but only the first 2 have been used."
	)
	...
	expect_warning(
		design(
			...
			nrows = 6,
			ncols = 4,
			fac.names = list(Water = c("A", "B"), N = 1:2, Another = 1),
			quiet = TRUE
		),
...
test_that("plot = FALSE does not produce a plot, but autoplot does", {
	...
	vdiffr::expect_doppelganger(
		title = "Plot produced with plot = FALSE",
		autoplot(d1),
		variant = ggplot2_variant()
	)
})
...
test_that("autoplot responds to margin argument", {
	...
	vdiffr::expect_doppelganger(
		title = "autoplot with margin",
		autoplot(d1, margin = TRUE),
		variant = ggplot2_variant()
	)
})
...
test_that("autoplot responds to rotation argument", {
	...
	vdiffr::expect_doppelganger(
		title = "autoplot with rotation",
		autoplot(d1, rotation = 90),
		variant = ggplot2_variant()
	)
})
...
test_that("autoplot responds to legend argument", {
	...
	vdiffr::expect_doppelganger(
		title = "autoplot no legend",
		autoplot(d1, legend = FALSE),
		variant = ggplot2_variant()
	)
})
...
test_that("Colour blind friendly plots work", {
	...
	expect_snapshot_output(d2$satab)
	vdiffr::expect_doppelganger(
		title = "CRD colour blind",
		autoplot(d1, palette = "colour blind"),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "RCBD colour blind",
		autoplot(d2, palette = "cb"),
		variant = ggplot2_variant()
	)
})
...
test_that("Colour blind friendly viridis", {
	...
	vdiffr::expect_doppelganger(
		title = "CRD colour blind viridis",
		autoplot(d1, palette = "viridis"),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "RCBD colour blind viridis",
		autoplot(d2, palette = "viridis"),
		variant = ggplot2_variant()
	)
})
...
test_that("Colour blind friendly magma", {
	...
	vdiffr::expect_doppelganger(
		title = "CRD colour blind magma",
		autoplot(d1, palette = "magma"),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "RCBD colour blind magma",
		autoplot(d2, palette = "magma"),
		variant = ggplot2_variant()
	)
})
...
test_that("Colour blind friendly inferno", {
	...
	vdiffr::expect_doppelganger(
		title = "CRD colour blind inferno",
		autoplot(d1, palette = "inferno"),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "RCBD colour blind inferno",
		autoplot(d2, palette = "inferno"),
		variant = ggplot2_variant()
	)
})
...
test_that("Colour blind friendly plasma", {
	...
	vdiffr::expect_doppelganger(
		title = "CRD colour blind plasma",
		autoplot(d1, palette = "plasma"),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "RCBD colour blind plasma",
		autoplot(d2, palette = "plasma"),
		variant = ggplot2_variant()
	)
})
...
test_that("Colour blind friendly cividis", {
	...
	vdiffr::expect_doppelganger(
		title = "CRD colour blind cividis",
		autoplot(d1, palette = "cividis"),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "RCBD colour blind cividis",
		autoplot(d2, palette = "cividis"),
		variant = ggplot2_variant()
	)
})
...
test_that("Various colour blind spellings and options", {
	...
	vdiffr::expect_doppelganger(
		title = "CRD colour blind option1",
		autoplot(d1, palette = "colour-blind"),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "CRD colour blind option2",
		autoplot(d1, palette = "colour blind"),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "CRD colour blind option3",
		autoplot(d1, palette = "colour_blind"),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "CRD colour blind option4",
		autoplot(d1, palette = "colour.blind"),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "CRD colour blind option5",
		autoplot(d1, palette = "colourblind"),
		variant = ggplot2_variant()
	)

	vdiffr::expect_doppelganger(
		title = "CRD color blind option1",
		autoplot(d1, palette = "color-blind"),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "CRD color blind option2",
		autoplot(d1, palette = "color blind"),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "CRD color blind option3",
		autoplot(d1, palette = "color_blind"),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "CRD color blind option4",
		autoplot(d1, palette = "color.blind"),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "CRD color blind option5",
		autoplot(d1, palette = "colorblind"),
		variant = ggplot2_variant()
	)

	vdiffr::expect_doppelganger(
		title = "RCBD colour blind option1",
		autoplot(d2, palette = "colour-blind"),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "RCBD color blind option1",
		autoplot(d2, palette = "color-blind"),
		variant = ggplot2_variant()
	)
})
...
test_that("Alternative palettes work", {
	...
	vdiffr::expect_doppelganger(
		title = "CRD RdBu palette",
		autoplot(d1, palette = "RdBu"),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "CRD Set3 palette",
		autoplot(d1, palette = "Set3"),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "CRD Paired palette",
		autoplot(d1, palette = "Paired"),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "RCBD RdBu palette",
		autoplot(d2, palette = "RdBu"),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "RCBD Set3 palette",
		autoplot(d2, palette = "Set3"),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "RCBD Paired palette",
		autoplot(d2, palette = "Paired"),
		variant = ggplot2_variant()
	)
})
...
test_that("Users can provide custom colours for the palette argument", {
	...
	vdiffr::expect_doppelganger(
		...
		autoplot(
			d1,
			palette = c("red", "blue", "orange", "darkgreen", "purple")
		),
		variant = ggplot2_variant()
	)
})
...
test_that("Adding buffers to plots works", {
	...
	vdiffr::expect_doppelganger(
		title = "Double row buffers",
		autoplot(d4),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "Double Column buffers",
		autoplot(d5),
		variant = ggplot2_variant()
	)
})
...
test_that("Adding buffers to plots works for RCBD", {
	...
	vdiffr::expect_doppelganger(
		title = "Row buffers RCBD",
		autoplot(d1),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "Column buffers RCBD",
		autoplot(d2),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "Edge buffers RCBD",
		autoplot(d3),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "Double row buffers RCBD",
		autoplot(d4),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "Double Column buffers RCBD",
		autoplot(d5),
		variant = ggplot2_variant()
	)
})
...
test_that("Ability to provide arbitrary column names for plotting works", {
	...
	vdiffr::expect_doppelganger(
		title = "Quoted column names without blocks",
		autoplot(des, row = "ro", column = "co", treatments = "treat"),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "Quoted column names with blocks",
		autoplot(des, row = "ro", column = "co", treatments = "treat"),
		variant = ggplot2_variant()
	)
})
...
test_that("Arbitrary unquoted column names for plotting works", {
	...
	class(des) <- c("design", class(des))
	vdiffr::expect_doppelganger(
		title = "NSE of column names without blocks",
		autoplot(des, row = ro, column = co, treatments = treat),
		variant = ggplot2_variant()
	)
	vdiffr::expect_doppelganger(
		title = "NSE of column names with blocks",
...
test_that("des_info() returns a CRD design without plotting", {
	...
	expect_design_output(
		out,
		expected_names = c("design", "satab", "seed"),
		expected_seed = 42,
		expect_plot = FALSE
	)
...
test_that("des_info() renames split plot factor columns when fac.names is a character vector", {
	...
	if (is.data.frame(split_obj$book)) {
		...
		if (length(candidates) >= 2) {
			...
			names(split_obj$book)[
				names(split_obj$book) == main_col
			] <- "c(\"A\", \"B\")"
