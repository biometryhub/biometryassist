...
if (nchar(vsni_url) == 0) {
	stop("VSNI_URL environment variable not set")
}
...
parse_version_table_html <- function(table_node, header) {
	...
	parsed <- lapply(df$Download, parse_download_os, r_ver = df$r_ver[1])
	...
	df$`Date published` <- as.Date(
		df$`Date published`,
		tryFormats = c("%d %B %Y", "%Y-%m-%d", "%Y/%m/%d")
	)
	...
	tryCatch(
		{
			...
			headers <- headers[grepl(
				"^ASReml-?R? 4.*\\(All platforms\\)",
				headers
			)]
			if (length(headers) == 0) {
				stop("No version info found at URL")
			}
...
packages_to_df <- function(packages) {
	...
	if (is.null(packages)) {
		...
		stop(
			"Unsupported packages type: ",
			paste(class(packages), collapse = ", ")
		)
	}
...
sync_asreml_shortlinks <- function(
	...
	current <- get_shortio_links(api_key, domain_id)[, c(
		"id",
		"path",
		"originalURL",
		"title"
...
if (!is.null(old_manifest)) {
	...
	manifest_changed <- !(identical(
		as.integer(old_schema),
		as.integer(new_schema)
	) &&
