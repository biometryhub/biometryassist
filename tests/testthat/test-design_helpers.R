...
test_that("construct_factorial_labels works with default separators", {
	...
	result <- biometryassist:::construct_factorial_labels(
		design_book,
		start_col = 3
	)
	...
	expect_equal(
		as.character(result),
		c("Aa1 Bb1", "Aa1 Bb2", "Aa2 Bb1", "Aa2 Bb2")
	)
...
test_that("construct_factorial_labels works with two custom separators", {
	...
	expect_equal(
		as.character(result),
		c("Variety:V1 x Fertilizer:F1", "Variety:V2 x Fertilizer:F2")
	)
	expect_s3_class(result, "factor")
})
...
test_that("construct_factorial_labels handles single factor column", {
	...
	expect_equal(
		as.character(result),
		c("treatmentT1", "treatmentT2", "treatmentT3")
	)
	expect_s3_class(result, "factor")
})
...
test_that("construct_factorial_labels handles three factors", {
	...
	expect_equal(
		as.character(result),
		c("A=a1, B=b1, C=c1", "A=a2, B=b2, C=c2")
	)
	expect_s3_class(result, "factor")
})
...
test_that("construct_factorial_labels handles various data types in columns", {
	...
	expect_equal(
		as.character(result),
		c(
			"dose:10|method:A|temp:20",
			"dose:20|method:B|temp:25",
			"dose:30|method:C|temp:30"
		)
	)
	expect_s3_class(result, "factor")
})
...
test_that("construct_factorial_labels works with empty string separators", {
	...
	expect_equal(
		as.character(result),
		c("Factor1XFactor21", "Factor1YFactor22")
	)
	expect_s3_class(result, "factor")
})
...
test_that("apply_factor_names throws error for unknown design_type", {
	...
	expect_error(
		apply_factor_names(
			design_book,
			fac.names = list(F1 = c("a", "b"), F2 = c("x", "y")),
			design_type = "unknown"
		),
...
test_that("calculate_block_layout returns correct dimensions for all paths", {
	...
	r4 <- biometryassist:::calculate_block_layout(
		6,
		6,
		3,
		3,
	...
	r5 <- biometryassist:::calculate_block_layout(
		3,
		6,
		3,
		2,
...
test_that("get_design_info correctly identifies non-factorial designs", {
	...
	crd_design <- agricolae::design.crd(
		trt = c("A", "B", "C"),
		r = 3,
		seed = 42
	)
...
test_that("get_design_info correctly identifies RCBD designs", {
	...
	rcbd_design <- agricolae::design.rcbd(
		trt = c("T1", "T2", "T3", "T4"),
		r = 4,
		seed = 42
	)
...
test_that("get_design_info correctly identifies LSD designs", {
	...
	lsd_design <- agricolae::design.lsd(
		trt = c("V1", "V2", "V3", "V4"),
		seed = 42
	)
...
test_that("get_design_info correctly identifies factorial designs with CRD", {
	...
	factorial_crd <- agricolae::design.ab(
		trt = c(2, 3),
		r = 2,
		design = "crd",
		seed = 42
...
test_that("get_design_info correctly identifies factorial designs with RCBD", {
	...
	factorial_rcbd <- agricolae::design.ab(
		trt = c(3, 2),
		r = 3,
		design = "rcbd",
		seed = 42
...
test_that("get_design_info correctly identifies factorial designs with LSD", {
	...
	factorial_lsd <- agricolae::design.ab(
		trt = c(2, 2),
		r = 1,
		design = "lsd",
		seed = 42
...
test_that("get_design_info handles 3-way factorial designs", {
	...
	factorial_3way <- agricolae::design.ab(
		trt = c(2, 2, 2),
		r = 2,
		design = "crd",
		seed = 42
...
test_that("validate_strip_inputs errors on invalid strip inputs", {
	...
	# Missing brows/bcols
	expect_error(
		biometryassist:::validate_strip_inputs(
			base_book,
			nrows = 2,
			ncols = 2,
			brows = NA,
