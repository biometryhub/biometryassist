...
#' @noRd
calculate_block_layout <- function(
	nrows,
	ncols,
	brows,
	bcols,
...
#' @noRd
construct_factorial_labels <- function(
	design_book,
	start_col,
	fac.sep = c("", " ")
) {
...
apply_factor_names <- function(
	...
	if (is.list(fac.names)) {
		for (i in seq_len(n_facs)) {
			...
			if (isTRUE(applied)) {
				if (design_type == "factorial") {
					colnames(design_book)[
						colnames(design_book) == col_name
					] <- names(fac.names)[i]
				} else if (design_type == "split") {
					colnames(design_book)[
						colnames(design_book) == col_name
		...
		colnames(design_book)[
			colnames(design_book) %in% factor_cols
		] <- fac.names[1:2]
	}
...
validate_strip_inputs <- function(design_book, nrows, ncols, brows, bcols) {
	...
	if (
		"block" %in%
			names(design_book) &&
			n_unique(design_book$block) != (rr * cc)
	) {
		stop(
...
#' @noRd
calculate_total_plots <- function(
	parsed_type,
	treatments,
	reps,
	sub_treatments
