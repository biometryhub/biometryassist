...
create_diagnostic_plots <- function(group_residuals, axes.size, label.size) {
	...
	group_scatter_clean <- group_residuals_clean[
		is.finite(group_residuals_clean$fitted) &
			!is.na(group_residuals_clean$fitted),
	]
...
shapiro_test <- function(group_residuals) {
	...
	output <- list(
		...
		text2 = ifelse(
			shap$p.value > 0.05,
			paste0(
				"The residuals appear to be normally distributed. (n = ",
				n,
				")"
			),
			paste0(
				"The residuals do not appear to be normally distributed. (n = ",
				n,
				")"
			)
		)
...
#' @keywords internal
format_output_resplot <- function(
	output,
	facet,
	facet_name,
	onepage,
