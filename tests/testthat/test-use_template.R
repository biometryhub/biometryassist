...
test_that("use_template warns and falls back to default if template file does not exist", {
	...
	withr::with_tempdir({
		expect_warning(
			dest <- use_template(
				template_name = "not_a_real_template.R",
				open = FALSE
			),
			"not found as a file"
		)
...
test_that("use_template opens file in editor if open = TRUE", {
	...
	withr::with_tempdir({
		...
		if (exists("opened", envir = .GlobalEnv)) {
			rm(opened, envir = .GlobalEnv)
		}
		# Mock file.edit in the correct environment
		mockery::stub(use_template, "utils::file.edit", function(path) {
			assign("opened", path, envir = .GlobalEnv)
			TRUE
		})
		use_template(open = TRUE)
		expect_true(exists("opened", envir = .GlobalEnv))
		expect_equal(
			normalizePath(get("opened", envir = .GlobalEnv)),
			normalizePath("analysis_script.R")
		)
		rm(opened, envir = .GlobalEnv)
	})
})
...
test_that("use_template input validation works", {
	expect_error(
		use_template(template_name = c("a", "b")),
		"must be a single character string"
	)
	expect_error(
		use_template(dest_dir = c("a", "b")),
		"must be a single character string"
	)
	expect_error(use_template(open = c(TRUE, FALSE)), "must be TRUE or FALSE")
	expect_error(
		use_template(overwrite = c(TRUE, FALSE)),
		"must be TRUE or FALSE"
	)
	expect_error(
		use_template(output_name = c("a", "b")),
		"must be a single character string"
	)
})
