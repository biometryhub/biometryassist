...
anova_factorial_crd <- function(design_book) {
	...
	if (length(trt_names) >= 2) {
		for (k in 2:length(trt_names)) {
			...
			interaction_sources <- c(
				interaction_sources,
				vapply(cmb, function(x) paste(x, collapse = ":"), character(1))
			)
			interaction_dfs <- c(
				interaction_dfs,
				vapply(cmb, function(x) prod(trtdf[x]), numeric(1))
			)
		}
	}
...
anova_factorial_rcbd <- function(design_book) {
	...
	list(
		...
		strata = list(
			block = 1,
			main = 2:(2 + length(trtdf) + length(interaction_dfs) + 1)
		)
	)
}
...
anova_split <- function(design_book) {
	...
	if (length(trt_cols) != 2) {
		stop(
			"Expected 2 treatment columns in split plot design, found ",
			length(trt_cols),
			call. = FALSE
		)
	}
...
anova_strip <- function(design_book) {
	...
	if (length(trt_cols) != 2) {
		stop(
			"Expected 2 treatment columns in strip plot design, found ",
			length(trt_cols),
			call. = FALSE
		)
	}
	...
	list(
		sources = c(
			"Block stratum",
			trtAname,
			paste0(trtAname, " Residual"),
			trtBname,
			paste0(trtBname, " Residual"),
			paste(trtAname, trtBname, sep = ":"),
...
format_satab <- function(anova_structure, design_type) {
	...
	for (i in seq_along(sources)) {
		...
		if (!is.null(anova_structure$strata) && i == 1) {
			output <- c(
				output,
				paste0(format(sources[i], width = 40), df[i], "\n")
			)
			output <- c(
				output,
				"---------------------------------------------\n"
...
format_satab_split <- function(anova_structure) {
	...
	output <- c(
		...
		paste0(
			format(" ", width = 9),
			format(sources[4], width = width3),
			df[4],
			"\n"
		),
		paste0(
			format(" ", width = 9),
			format(sources[5], width = width4),
			df[5],
			"\n"
		),
		paste0(
			format(" ", width = 9),
			format(sources[6], width = 35),
			df[6],
			"\n"
		),
		"==================================================\n",
