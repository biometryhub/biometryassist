...
- Visual/plot regression tests rely on snapshot SVGs under `tests/testthat/_snaps/`. Snapshots are split by ggplot2 version (`ggplot2-new` vs `ggplot2-old`) — see `ggplot2_variant()` in `tests/testthat/helper-expectations.R`.
- **NEVER accept a failing snapshot without the user reviewing it first.** A snapshot test fails because output changed; blindly running `snapshot_accept()` (or accepting in bulk) makes the test pass while potentially baking in broken output — defeating the entire point of the test. The required workflow when a snapshot fails: (1) show the user *what* changed (e.g. `testthat::snapshot_review()`, or diff the `.new.svg` against the committed `.svg`), (2) explain why you believe the change is correct, (3) let the user confirm, and only then accept. The user must review snapshot changes — this is not a decision to make on their behalf, even when the diff looks minor or you have a plausible explanation. Do not run `snapshot_accept()` unprompted.
...
- Loose `.R` scripts in the repo root (e.g. `*SamVariogram.R`, `sommer_vs_asreml*.R`, `asreml_*.R`) are scratch/research files, not package code — they are listed in `.Rbuildignore` and excluded from the build.
