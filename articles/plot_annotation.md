...
[`annotation_north_arrow()`](https://paleolimbot.github.io/ggspatial/reference/annotation_north_arrow.html)
...
- `style = north_arrow_fancy_orienteering()`: Specifies the visual style
  ...
  - [`north_arrow_minimal()`](https://paleolimbot.github.io/ggspatial/reference/north_arrow_orienteering.html):
    Simple arrow design
  - [`north_arrow_nautical()`](https://paleolimbot.github.io/ggspatial/reference/north_arrow_orienteering.html):
    Compass-style design
  - [`north_arrow_orienteering()`](https://paleolimbot.github.io/ggspatial/reference/north_arrow_orienteering.html):
    Classic orienteering map style
...
- `theme(plot.margin = ...)`: Expands the margins around the plot to
  create space for the arrow. The
  [`margin()`](https://ggplot2.tidyverse.org/reference/element.html)
  function takes four values: top, right, bottom, left. Units are
  specified with the `unit` parameter (here, points). We increase the
  right margin to `50pt` to accommodate the arrow.
...
function allows us to add text (and other geometric objects) to specific
locations on the plot. Unlike the north arrow, these annotations use the
...
**Solution**: When using
[`annotate()`](https://ggplot2.tidyverse.org/reference/annotate.html)
with coordinates outside the plot range, always specify explicit limits
in `coord_cartesian(xlim = ..., ylim = ...)` to prevent automatic axis
expansion.
...
values to create space for external elements - For
[`annotate()`](https://ggplot2.tidyverse.org/reference/annotate.html),
