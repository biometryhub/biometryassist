...
[![Codecov test coverage](https://codecov.io/gh/biometryhub/biometryassist/branch/master/graph/badge.svg)](https://app.codecov.io/gh/biometryhub/biometryassist?branch=master)
...
[![packageversion](https://img.shields.io/badge/Package%20version-`r gsub('-', '--', version)`-orange.svg?style=flat-square)](https://github.com/biometryhub/biometryassist/commits/master)
...
If you find this pacakge useful, please cite it! Type `citation("biometryassist")` on the R console to find out how.
...
- If you receive an error that the package could not install because `rlang` or another package could not be upgraded, the easiest way to deal with this is to uninstall the package(s) that could not be updated (`remove.packages("rlang")`). Then reinstall with `install.packages("rlang")` and then try installing `biometryassist` again.
